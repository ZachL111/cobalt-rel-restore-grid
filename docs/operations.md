...
`cobalt-rel-restore-grid` is centered on reliability work in Solidity. The technical target is: Develop a Solidity command-oriented project for restore scenarios with transition tables, invalid-transition tests, and local-only command execution.
...
The score is deterministic. The current threshold is 168. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 2.
